...
Worker setup (if not using Docker Compose):
  pip install -r requirements.txt
  prefect work-pool create default-agent-pool --type process
  prefect worker start --pool default-agent-pool

Register deployments:
  python flows/deploy_local.py
  python flows/deploy_local.py --dry-run

Or from Docker:
  docker-compose exec worker python flows/deploy_local.py
"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Deploy market data flows to local Prefect server")
    parser.add_argument(
        "--work-pool",
